...
function DashboardContent() {
  ...
  // OAuth callback & error toast handler
  useEffect(() => {
    ...
    if (connected === 'true' && email) {
      ...
      if (error === 'unauthenticated') {
        ...
        errorText = 'Google did not return a refresh token. Revoke access in Google security settings and try again.';
      }
  ...
  const handleRebalanceStorage = async () => {
    ...
    try {
      ...
      if (res.ok) {
        refreshAllData();
        setToastMessage({ text: json.data?.message || 'Storage rebalancing completed', type: 'success' });
      } else {
        setToastMessage({ text: json.error?.message || 'Rebalance failed', type: 'error' });
      }
    } catch (err) {
  ...
  const handleDeleteFile = async (fileId: string) => {
    try {
      const res = await fetch('/api/files/batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete', fileIds: [fileId] }),
      });
      if (res.ok) {
        refreshAllData();
...
export default function MultiDriveDashboardPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-950 text-slate-400 p-8 text-center text-xs">Loading MultiDrive Dashboard...</div>}>
      <DashboardContent />
    </Suspense>
  );
}
