  ...
  try {
    ...
    if (action === 'bulk_move') {
      const { error } = await supabase
        .from('file_records')
        .update({
          virtual_folder_id: targetFolderId || null,
        })
        .in('id', fileIds);
    ...
    if (action === 'bulk_permanent_delete') {
      ...
      await supabase.from('file_records').delete().in('id', fileIds);

      // Refresh affected account quotas asynchronously
      for (const accId of Array.from(affectedAccountIds)) {
        const { data: acc } = await supabase.from('connected_accounts').select('*').eq('id', accId).single();
        if (acc) {
          try {
